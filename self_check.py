...
def main(argv: list[str] | None = None) -> int:
    ...
    print("  seed     direct  PI=I    agent    delta          base   agent   reduction")
    print("  " + "-" * 70)
    for r in report["per_seed"]:
        flag = "Y" if r["dynamics_adds_value"] else "N"
        print(f"  {r['seed']:>4}    {r['direct_classify_acc']:.3f}  "
              f"{r['baseline_acc']:.3f}  {r['agent_acc']:.3f}  "
              f"{r['delta']:+.3f} {flag}    "
              f"{r['baseline_spread']:>6.2f}  {r['agent_spread']:>6.2f}  "
              f"{r['spread_reduction']:>5.2f}x")
    print()
    print("  AGGREGATED                                  VALUE")
    print("  " + "-" * 70)
    print(f"  mean delta accuracy (over seeds)               {agg['mean_delta']:+.3f}")
    print(f"  min  delta accuracy (worst seed)               {agg['min_delta']:+.3f}")
    print(f"  mean spread reduction                      {agg['mean_reduction']:>6.2f}x")
    print(f"  min  spread reduction                      {agg['min_reduction']:>6.2f}x")
    ...
    if sc["notes"]:
        print("  NOTES")
        for n in sc["notes"]:
            sanitized_n = n.replace('\u03a0', 'PI').replace('\u2014', '-')
            print(f"    - {sanitized_n}")
        print()
